...
import { signer, desinscrire, lienDesinscription, envoyerUn } from "./lib/abonnes.js";
import { emailBienvenue, sujetBienvenue, adresseReponse } from "./lib/email.js";
...
async function inscription(request, env, ctx) {
  ...
  try {
    ...
    if (res.status === 201 || res.status === 204) {
      // L'email de bienvenue part en arrière-plan : la page répond tout de suite.
      if (ctx && ctx.waitUntil) ctx.waitUntil(envoyerBienvenue(env, email));
      else await envoyerBienvenue(env, email);
...
async function envoyerBienvenue(env, email) {
  const urlSite = (env.SITE_URL || "").replace(/\/$/, "");
  const secret = env.UNSUB_SECRET || env.TEST_TOKEN;

  if (!env.BREVO_API_KEY || !env.SENDER_EMAIL || !urlSite || !secret) {
    console.error("Bienvenue non envoyée : configuration incomplète.");
    return;
  }

  try {
    const lienDesabo = await lienDesinscription(email, urlSite, secret);
    await envoyerUn({
      ...
      email,
      sujet: sujetBienvenue(),
      html: emailBienvenue(urlSite, lienDesabo),
      lienDesabo,
    });
    console.log(`Bienvenue envoyée à ${email}`);
  } catch (e) {
    // Une bienvenue qui échoue ne doit jamais faire échouer l'inscription.
    console.error(`Bienvenue non envoyée à ${email} : ${e.message}`);
  }
}
